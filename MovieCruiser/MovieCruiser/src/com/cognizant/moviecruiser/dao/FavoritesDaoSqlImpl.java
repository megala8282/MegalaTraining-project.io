...
public class FavoritesDaoSqlImpl implements FavoritesDao {
	...
	public static final String GET_NUMBER_OF_FAVORITES = "select count(*) as Number_of_favorites from favorites WHERE ft_us_id=?";
	...
	public Favorites getAllFavoritesMovies(long userId) throws FavoritesEmptyException {
		...
		try {
			...
			preparedStatementTotal = connection.prepareStatement(GET_NUMBER_OF_FAVORITES);
			...
			while (resultSetTotal.next()) {
				favorites.setTotal(resultSetTotal.getInt("Number_of_favorites"));
			}
